...
class Config:
    ...
    def _setup_properties(self) -> None:
        ...
        self.PURCHASE_ONLY_UPGRADABLE_GIFTS = self.parser.getboolean('Gifts', 'PURCHASE_ONLY_UPGRADABLE_GIFTS',
                                                                     fallback=False)
        self.PURCHASE_NON_LIMITED_GIFTS = self.parser.getboolean('Gifts', 'PURCHASE_NON_LIMITED_GIFTS',
                                                                 fallback=False)
    ...
    def _parse_single_recipient(self, recipient: str) -> Union[int, str, None]:
        ...
        # Если начинается с @, убираем @ и возвращаем username
        ...
        # Если начинается с -, это может быть отрицательный ID чата
        if recipient.startswith('-') and recipient[1:].isdigit():
            return int(recipient)
        
        # В остальных случаях возвращаем как строку (username без @)
        return recipient



    def get_matching_range(self, price: int, total_amount: int, is_limited: bool = True) -> tuple[bool, int, List[Union[int, str]]]:
        matching_ranges = [
            ...
            if (range_config['min_price'] <= price <= range_config['max_price'] and
                (not is_limited or total_amount <= range_config['supply_limit']))
        ]
