...
from app.purchase import GiftPurchaser
...
class GiftProcessor:
    ...
    async def evaluate_gift(gift_data: Dict[str, Any]) -> tuple[bool, Dict[str, Any]]:
        ...
        exclusion_rules = {
            ...
            'non_limited_blocked': lambda: not is_limited and not config.PURCHASE_NON_LIMITED_GIFTS,
        ...
        return (False, {'exclusion_reason': failed_rule}) if failed_rule else \
            GiftProcessor._evaluate_range_match(gift_price, total_amount, is_limited)

    @staticmethod
    def _evaluate_range_match(gift_price: int, total_amount: int, is_limited: bool) -> tuple[bool, Dict[str, Any]]:
        range_matched, quantity, recipients = config.get_matching_range(gift_price, total_amount, is_limited)
...
async def _distribute_gifts(app: Client, gift_id: int, quantity: int, recipients: list) -> None:
    ...
    for recipient_id in recipients:
        try:
            await GiftPurchaser.buy_gift(app, recipient_id, gift_id, quantity)
