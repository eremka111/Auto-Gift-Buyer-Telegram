...
class NotificationManager:
    ...
    async def send_start_message(client: Client) -> None:
        ...
        def format_range(r):
            if config.PURCHASE_NON_LIMITED_GIFTS:
                supply_info = "любой лимит" if config.LANGUAGE == "ru" else "any limit"
            else:
                supply_info = f"≤ {r['supply_limit']}"
            return f"• {r['min_price']}-{r['max_price']} ⭐ (supply {supply_info}) x{r['quantity']} -> {len(r['recipients'])} recipients"
        
        ranges_text = "\n".join([format_range(r) for r in config.GIFT_RANGES])
